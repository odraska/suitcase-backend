...
/**
 * Font Family Page GraphQL Resolver
 *
 * @author    Lubos Odraska <odraska@slonline.sk>
 * @copyright Copyright (c) 2025, SLONline, s.r.o.
 */
class FontFamilyPageResolver
{
    public static function resolveCategoryIDFilter(SS_List $list, array $args, array $context)
    {
        if ($list === null) {
            return null;
        }
        ...
        if ($filter) {
            $list = $filter->apply($list, 'FontCategories.ID', $context['filterValue']);
        }
    ...
    public static function resolveCategoryUrlSegmentFilter(SS_List $list, array $args, array $context)
    {
        ...
        if ($filter) {
            $list = $filter->apply($list, 'FontCategories.UrlSegment', $context['filterValue']);
        }
